<template>
    <section id='foot_guide'>
        ...
        <section @click = "gotoAddress({path: '/XSRmsite/XSRrecommend'})" class="guide_item">
        ...
        methods: {
            ...
            goCare(){//点击关心
                this.$http.get("static/error.json").then(
                    function(res){
                        ...
                        if(listData.error==1){
                            this.$router.push({path: '/XSRcareaddFinished'})//进入未登录进入页
                        }else{
                            this.$router.push({path: '/XSRcareEntering'})//进入标签列表页
                        }
                    },
                    function(){
                        console.log("数据请求失败")
            ...
            goMe(){
            	...
            	if(WxOAuth2Token!="" && WxOAuth2Token!=null){
            		this.$router.push({path: '/XSRmeInfoprofile'})//进入已登录'我'页
            	}else{
            		this.$router.push({path:'/XSRmeInfohaventlogin'})
            	}
