<template>
    <section id='foot_guide'>
        ...
        <section @click = "gotoAddress({path: '/JKCSmsite/JKCSrecommend'})" class="guide_item">
        	<!--<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('msite') !== -1? '#msiteActive' : '#msite'"></use>
            </svg>-->
            <span class="tabIcon" :class="$route.path.indexOf('msite') !== -1? 'knowledgeLight' : 'knowledgeDark'"></span>
            <span>知识</span>
        </section>
        <section @click = "goCare()" class="guide_item">
        	<!--<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('careEntering') !== -1? '#findActive' : '#find'"></use>
            </svg>-->
            <span class="tabIcon" :class="$route.path.indexOf('care') !== -1? 'careLight' : 'careDark'"></span>
            <span>关心</span>
        </section>
        <section @click = "goMe()" class="guide_item">
        	<!--<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('profile') !== -1? '#profileActive' : '#profile'"></use>
            </svg>-->
            <span class="tabIcon" :class="$route.path.indexOf('meInfo') !== -1? 'meLight' : 'meDark'"></span>
            <span>我</span>
        </section>
...
//  import {mapState} from 'vuex'
    export default {
        ...
        methods: {
            ...
            goCare(){//点击关心
                this.$http.get("static/error.json").then(
                    function(res){
                        var listData=res.data
                        //console.log(listData)
                        if(listData.error==1){
                            this.$router.push({path: '/JKCScareaddFinished'})//进入未登录进入页
                        }else{
                            this.$router.push({path: '/JKCScareEntering'})//进入标签列表页
                        }
                    },
                    function(){
                        console.log("数据请求失败")
            ...
            goMe(){
            	...
            	console.log(WxOAuth2Token)
            	if(WxOAuth2Token!="" && WxOAuth2Token!=null){
            		this.$router.push({path: '/JKCSmeInfoprofile'})//进入已登录'我'页
            	}else{
            		this.$router.push({path:'/JKCSmeInfohaventlogin'})
            	}
            	//this.$router.push({path: '/meInfoprofile'})//进入已登录'我'页
            	/*
                this.$http.get("static/error.json").then(
                    function(res){
                        var listData=res.data
                        //console.log(listData)
                        if(listData.error==1){
                            this.$router.push({path: '/meInfohaventlogin'})//进入用户登录页
                        }else{
                            this.$router.push({path: '/meInfoprofile'})//进入已登录'我'页
                        }
                    },
...
<style lang="scss" scoped>
    ...
    .guide_item{
		...
		span{
			font-weight:600;
			@include sc(.45rem, #929292);
		}
    }
   .tabIcon{width: 0.8rem;height: 0.8rem;margin-top: .3rem;}
   .knowledgeDark{background: url(/static/MY/img/btn_nav_ZhiShi@2x.png);background-size: cover;}
   .knowledgeLight{background: url(/static/MY/img/btn_hig_ZhiShi@2x.png);background-size: cover;}
   .careDark{background: url(/static/MY/img/btn_nav_GuanXin@2x.png);background-size: cover;}
   .careLight{background: url(/static/MY/img/btn_hig_GuanXin@2x.png);background-size: cover;}
   .meDark{background: url(/static/MY/img/btn_nav_GeRen@2x.png);background-size: cover;}
   .meLight{background: url(/static/MY/img/btn_hig_GeRen@2x.png);background-size: cover;}
</style>
