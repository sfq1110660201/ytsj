<template>
    <section id='foot_guide'>
        ...
        <section @click = "gotoAddress({path: '/msite/recommend'})" class="guide_item">
        ...
        methods: {
            ...
            goCare(){//点击关心
                this.$http.get("static/error.json").then(
                    function(res){
                        ...
                        if(listData.error==1){
                            this.$router.push({path: '/careaddFinished'})//进入未登录进入页
                        }else{
                            this.$router.push({path: '/careEntering'})//进入标签列表页
                        }
                    },
                    function(){
                        console.log("数据请求失败")
            ...
            goMe(){
            	...
            	if(WxOAuth2Token!="" && WxOAuth2Token!=null){
            		...
            		this.$router.push({path:'/meInfohaventlogin'})
            	}
            	//this.$router.push({path: '/meInfoprofile'})//进入已登录'我'页
            	/*
                this.$http.get("static/error.json").then(
