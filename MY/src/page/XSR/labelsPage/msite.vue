<template>
    <div>
    	...
    	<head-top signin-up='msite' >
    		<img slot="logo" src="/static/MY/img/muyinglogo.png" class="logoImg"/>
    		<span class="logoTitle" slot="msite-title">新生儿</span>
			<input type="search" id="searchInput" slot="search" class="infoSearch" placeholder="搜索"   @focus="search()"/>
    	</head-top>
    	<!--头部导航-->
    	<ul class="headNav">
    		<div class="headNavContainer">
    			<li><router-link to="/XSRmsite/XSRrecommend" :class="{ lightColor:$route.path.indexOf('recommend') !== -1 }" >推荐</router-link> </li>
	    		<li><router-link to="/XSRmsite/XSRbeforepregnant/XSRdiet" :class="{ lightColor:$route.path.indexOf('beforepregnant') !== -1 }">孕前</router-link></li>
	    		<li><router-link to="/XSRmsite/XSRpregnanting/XSRdiet" :class="{ lightColor:$route.path.indexOf('pregnanting') !== -1 }">孕中</router-link></li>
	    		<li><router-link to="/XSRmsite/XSRrecovery/XSRdiet" :class="{ lightColor:$route.path.indexOf('recovery') !== -1 }">恢复</router-link></li>
    		</div>
    	</ul>
...
<script>
import headTop from 'src/components/header/head_XSR'
import footGuide from 'src/components/footer/footGuide_XSR'
...
export default {
    ...
    methods: {
		...
		enter(value){//模态窗搜索回车
			//debugger
			this.$router.push({path:"/XSRmsite/XSRrecommend"})
