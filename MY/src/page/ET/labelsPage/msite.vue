<template>
    <div>
    	...
    	<head-top signin-up='msite' >
    		...
    		<span class="logoTitle" slot="msite-title">儿童</span>
			<input type="search" id="searchInput" slot="search" class="infoSearch" placeholder="搜索"   @focus="search()"/>
    	</head-top>
    	<!--头部导航-->
    	<ul class="headNav">
    		<div class="headNavContainer">
    			<li><router-link to="/ETmsite/ETrecommend" :class="{ lightColor:$route.path.indexOf('recommend') !== -1 }" >推荐</router-link> </li>
	    		<li><router-link to="/ETmsite/ETbeforepregnant/ETdiet" :class="{ lightColor:$route.path.indexOf('beforepregnant') !== -1 }">孕前</router-link></li>
	    		<li><router-link to="/ETmsite/ETpregnanting/ETdiet" :class="{ lightColor:$route.path.indexOf('pregnanting') !== -1 }">孕中</router-link></li>
	    		<li><router-link to="/ETmsite/ETrecovery/ETdiet" :class="{ lightColor:$route.path.indexOf('recovery') !== -1 }">恢复</router-link></li>
    		</div>
    	</ul>
...
export default {
    ...
    methods: {
		...
		enter(value){//模态窗搜索回车
			//debugger
			this.$router.push({path:"/ETmsite/ETrecommend"})
